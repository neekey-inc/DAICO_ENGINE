...
export default {
  head() {
    return {
      name: "Edit Company_data"
    };
  },
  async asyncData({ $axios, params }) {
    try {
      let company_data= await $axios.$get(`company_data/${params.id}`);
      return { company_data };
    } catch (e) {
      return { company_data: [] };
    }
  ...
  methods: {
    ...
    async submitCompany_data() {
      let editedCompany_data = this.company_data;
      ...
      for (let data in editedCompany_data) {
        formData.append(data, editedCompany_data[data]);
      }
      try {
        let response = await this.$axios.$patch(
          `company_data/${editedCompany_data.id}/`,
          formData,
          config
        );
